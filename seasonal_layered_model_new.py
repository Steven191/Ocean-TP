...
class SeasonalLayeredModel:
    ...
    def train_seasonal_layer_model(self, season_data: pd.DataFrame, season: str) -> Dict:
        ...
        # 准备数据
        pressure = season_data['depth'].values  # 使用深度作为压力近似
        temperature = season_data['temp'].values
        ...
        for i in range(len(boundaries) - 1):
            ...
            if len(train_idx) == 0:
                 train_idx = indices
                 val_idx = indices # Fallback validation on train
    ...
    def _compute_global_stats(self, seasonal_data: Dict[str, pd.DataFrame]) -> Dict:
        ...
        for season_df in seasonal_data.values():
            all_temps.extend(season_df['temp'].values)
            all_pressures.extend(season_df['depth'].values)
    ...
    def evaluate(self, data_dir: Path, test_samples: int = 10000) -> Dict:
        ...
        test_df = load_all_data(str(data_dir))
        ...
        for season, season_df in seasonal_test_data.items():
            ...
            pressure = season_df['depth'].values
            true_temp = season_df['temp'].values
